...
public class Monthly extends JPanel {
	...
	public Monthly() {
		...
		JLabel lblNewLabel = new JLabel("Monthly");
		lblNewLabel.setFont(new Font("Tahoma", Font.BOLD, 22));
		lblNewLabel.setBounds(179, 11, 210, 31);
		panel.add(lblNewLabel);
		
        JDateChooser dateChooser=new JDateChooser();
        dateChooser.setBounds(189, 127, 200, 30);
        add(dateChooser);
        
        JDateChooser dateChooser1 = new JDateChooser();
		dateChooser1.setBounds(473, 127, 200, 30);
		add(dateChooser1);
		...
		srcbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String b=df.format(dateChooser1.getDate());
					//Date a=dateChooser.getDate();
					Statement s = db.mycon().createStatement();
					ResultSet r = s.executeQuery("select * from sampledb.biller where datetime between '"+a+"%' and '"+b+"%'");
		...
		lblNewLabel_1.setFont(new Font("Kokila", Font.BOLD, 21));
		lblNewLabel_1.setBounds(417, 127, 46, 30);
		add(lblNewLabel_1);
